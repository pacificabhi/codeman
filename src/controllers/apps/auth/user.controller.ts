...
import { 
    ...
    loginUser,
    verifyJwtToken
} from "../../../services/apps/auth/user.service"
...
const registerUserHandler = expressAsyncHandler(async (req, res) => {

    const { firstname, lastname, email, password } = req.body
    const userAvailable = await getUser(email)
    
    if(userAvailable) {
        throw new ResponseError(errorNames.VALIDATION_ERROR, "Email already Registered", errorCodes.VALIDATION_ERROR)
    }
...
const loginUserHandler = expressAsyncHandler(async (req, res) => {
    ...
    const { accessToken, refreshToken } = await loginUser(authenticatedUser)
...
const refreshAccessTokenHandler = expressAsyncHandler(async (req, res) => {
    const oldRefreshToken  = req.body.refreshToken
    const decoded: any = verifyJwtToken(oldRefreshToken)
